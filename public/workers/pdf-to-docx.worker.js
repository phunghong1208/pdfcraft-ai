...
async function init() {
  ...
  // pdf2docx only — no custom PyMuPDF→docx fallback
  pyodide.runPython(`
import os
import fitz  # PyMuPDF (dependency of pdf2docx)

# CMYK / exotic colorspace fix for pdf2docx image extraction
...
def pdf_convert():
    """Convert PDF → DOCX using pdf2docx library only."""
...
self.onmessage = async (event) => {
  ...
  try {
    ...
    if (type === 'convert') {
      ...
      self.postMessage({
        type: 'progress',
        message: 'Converting with pdf2docx...',
        percent: 10
      });
