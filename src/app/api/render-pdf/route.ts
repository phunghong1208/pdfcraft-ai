...
const PROXY_TIMEOUT_MS = Number(process.env.RENDER_PROXY_TIMEOUT_MS || '300000');
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const upstream = await fetch(`${UPSTREAM}/render`, {
      method: 'POST',
      body: outgoing,
    ...
    return NextResponse.json(
      {
        detail: isTimeout
          ? `Render server không phản hồi trong ${PROXY_TIMEOUT_MS / 1000}s.`
          : `Render proxy lỗi: ${message}`,
      },
