#!/usr/bin/env node
/**
 * Translate workspace fragment keys that still match English.
 */
...
for (const locale of LOCALES) {
  ...
  if (keys.length === 0) {
    console.log(`${locale}: workspace fragment up to date`);
    continue;
  }

  console.log(`${locale}: translating ${keys.length} workspace keys`);
  let done = 0;
  ...
  console.log(`${locale}: saved ${done} workspace translations`);
}

console.log('Workspace fragment translation complete.');
